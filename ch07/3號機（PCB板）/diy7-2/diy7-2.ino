...
unsigned long debounceDelay = 50;   // 按鍵彈跳延遲時間
unsigned long lastDebounceTime = 0; // 上次彈跳時間
bool buttonState;             // 暫存啟動鍵接腳的狀態
bool lastButtonState = LOW;   // 啟動鍵接腳的前次狀態

QEncoder2 enc(25, 32); // 定義「左編碼器」物件
...
void readSerial() {   // 讀取序列輸入值
  ...
  if (Serial.available()) {   // 若有序列資料輸入…
    ...
    Serial.printf("decayMode= %d, pwmFreq= %d\n",
                  _decay, _pwmFreq);
...
void loop() {
  ...
  static bool paused = true;    // 是否為暫停狀態，預設「是」
  static bool btnState = HIGH;  // 按鍵初設為高電位

  bool btnValue = digitalRead(BTN_PIN);  // 讀取按鍵的值
  if (btnValue != lastButtonState) {
    lastDebounceTime = millis();  // 重設彈跳時間
  }

  // 若超過彈跳延遲時間…
  if ((millis() - lastDebounceTime) > debounceDelay) {
    // 如果按鍵狀態已改變…
    if (btnValue != buttonState) {
      buttonState = btnValue;

      // 只有當按鍵狀態為LOW時才翻轉paused值
      if (buttonState == LOW) {
        paused = !paused;
      }
    }
