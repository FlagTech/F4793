...
public:
	...
	Cdrv8833(uint8_t in1Pin, uint8_t in2Pin, uint8_t channel, float offset = 0, bool swapDirection = false);
	~Cdrv8833();
	...
	bool init(uint8_t in1Pin, uint8_t in2Pin, uint8_t channel, float offset = 0, bool swapDirection = false);
	...
	// return:
	//   true if no error occurs.
	bool move(float power); // use fast decay -> smooth movement
