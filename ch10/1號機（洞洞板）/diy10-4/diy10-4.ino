...
#define LINE_MAX     438
#define BTN_PIN      27    // 「啟動」鍵的接腳

Button button(BTN_PIN);    // 「啟動」鍵物件
...
void run() {
  ...
  if (linePos == 0) {  // 此處的0代表「直行」
    ...
    //slowPWM = pwm.slow + (LINE_MAX - linePos) * pwm.s_scale; // 低速PWM
    if (fastPWM > 100) fastPWM = 100;
    motorL.move(pwm.slow);   // 左輪
    motorR.move(fastPWM);      // 右輪加速
    OLED(linePos, pwm.slow, fastPWM);
  } else {   // 右轉
    fastPWM = pwm.normal - linePos * pwm.scale;     // linePos要轉正
    // slowPWM = pwm.slow + (LINE_MAX + linePos + 1) * pwm.s_scale; // linePos是負值
    if (fastPWM > 100) fastPWM = 100;
